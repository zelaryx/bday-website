...
function togglePopup() {
	document.getElementById("popupletter").classList.toggle("active");
}
window.addEventListener('scroll', function () {
	var letterContent = document.querySelector('.content');
	var windowHeight = window.innerHeight;
	var scrollY = window.scrollY / windowHeight;

	// Adjust the top position based on the scroll position
	letterContent.style.top = 50 + scrollY + '%';
});

function checkPassword() {
	document.getElementById('gift-password-form').addEventListener('submit', function(event) {
		...
		if (passwordInput === "041523") {
			...
			document.getElementById("gift-password-form").reset();
		}
	});
}
